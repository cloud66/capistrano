		...
		def environment
			if @environment.nil?
				if options[:env].nil? || options[:env].empty?
					@environment = 'umask 0002 || true && . /var/.cloud66_env 2>/dev/null || true && unset BUNDLE_GEMFILE && '
				else
					@environment = if String === options[:env]
									   "umask 0002 || true && . /var/.cloud66_env 2>/dev/null || true && unset BUNDLE_GEMFILE || true && env #{options[:env]}"
								   else
									   env_val = options[:env].inject("env") do |string, (name, value)|
										   value = value.to_s.gsub(/[ "]/) { |m| "\\#{m}" }
										   string << " #{name}=#{value}"
									   end
									   "umask 0002 || true && . /var/.cloud66_env 2>/dev/null || true && unset BUNDLE_GEMFILE && #{env_val}"
								   end
				end
